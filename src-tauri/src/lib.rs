...
async fn download_file(client: &reqwest::Client, url: &str, path: &Path, expected_hash: Option<&str>) -> Result<(), String> { if path.exists() { if let Some(hash) = expected_hash { let existing = fs::read(path).map_err(|e| e.to_string())?; if sha1(&existing) == hash { return Ok(()); } } else { return Ok(()); } } if let Some(parent) = path.parent() { fs::create_dir_all(parent).map_err(|e| e.to_string())?; } let bytes = client.get(url).send().await.map_err(|e| format!("Network error downloading {}: {}", url, e))?.error_for_status().map_err(|e| e.to_string())?.bytes().await.map_err(|e| e.to_string())?; if let Some(hash) = expected_hash { if sha1(&bytes) != hash { return Err(format!("Checksum mismatch while downloading {}", url)); } } fs::write(path, &bytes).map_err(|e| e.to_string()) }
...
fn substitute(mut s: String, game_dir: &Path, assets_dir: &Path, asset_index: &str, natives_dir: &Path, instance: &str, version: &str, username: &str, uuid: &str, access_token: &str, user_type: &str) -> String { let game_dir_s = game_dir.to_string_lossy().to_string(); let assets_dir_s = assets_dir.to_string_lossy().to_string(); let natives_dir_s = natives_dir.to_string_lossy().to_string(); let library_dir_s = game_dir.join("libraries").to_string_lossy().to_string(); let replacements = [("${auth_player_name}", username),("${auth_uuid}",uuid),("${auth_access_token}",access_token),("${user_type}",user_type),("${version_name}",version),("${version_type}","release"),("${assets_root}",assets_dir_s.as_str()),("${assets_index_name}",asset_index),("${game_directory}",game_dir_s.as_str()),("${natives_directory}",natives_dir_s.as_str()),("${library_directory}",library_dir_s.as_str()),("${classpath_separator}",if cfg!(target_os="windows"){";"}else{":"}),("${launcher_name}","BlockPilot"),("${launcher_version}","0.1.0"),("${clientid}",""),("${auth_xuid}",""),("${instance_name}",instance)]; for (from,to) in replacements { s=s.replace(from,to); } s }
fn collect_args(value: &Value, game_dir: &Path, assets_dir: &Path, asset_index: &str, natives_dir: &Path, instance: &str, version: &str, username: &str, uuid: &str, access_token: &str, user_type: &str) -> Vec<String> { let mut out=Vec::new(); let Some(args)=value.as_array() else{return out;}; for entry in args { if !rules_allow(entry){continue;} let raw=entry.get("value").unwrap_or(entry); match raw { Value::String(s)=>out.push(substitute(s.clone(),game_dir,assets_dir,asset_index,natives_dir,instance,version,username,uuid,access_token,user_type)), Value::Array(values)=>for v in values.iter().filter_map(Value::as_str){out.push(substitute(v.to_string(),game_dir,assets_dir,asset_index,natives_dir,instance,version,username,uuid,access_token,user_type));}, _=>{} } } out }
...
/// Maven coordinate "group:artifact:version[:classifier]" -> relative jar path
...
fn emit_progress(app: &tauri::AppHandle, instance: &str, stage: &str, detail: &str) {
    let _ = app.emit("launch-progress", serde_json::json!({ "instance": instance, "stage": stage, "detail": detail }));
}
...
async fn launch_instance(app: tauri::AppHandle, instance: String) -> Result<String, String> {
    ...
    let (mc_version, loader, loader_version_pref) = management::read_profile(&instance_dir);
    ...
    emit_progress(&app, &instance, "manifest", "Checking Mojang's version list…");
    let manifest = fetch_json(&client, VERSION_MANIFEST).await?;
    let version_url = manifest.get("versions").and_then(Value::as_array)
        .and_then(|v| v.iter().find(|x| x.get("id").and_then(Value::as_str) == Some(mc_version.as_str())))
        .and_then(|x| x.get("url")).and_then(Value::as_str)
        .ok_or_else(|| format!("Minecraft {} was not found in Mojang's manifest", mc_version))?;
    ...
    emit_progress(&app, &instance, "client", &format!("Downloading Minecraft {} client…", mc_version));
    ...
    if let Some(libs) = meta.get("libraries").and_then(Value::as_array) {
        ...
        for (i, lib) in libs.iter().enumerate() {
            ...
            if i % 15 == 0 { emit_progress(&app, &instance, "libraries", &format!("Downloading libraries… {}/{}", i + 1, total)); }
    ...
    let asset_id = asset_index.get("id").and_then(Value::as_str).unwrap_or(mc_version.as_str()).to_string();
    let asset_index_file = assets_dir.join("indexes").join(format!("{}.json", asset_id));
    emit_progress(&app, &instance, "assets", "Fetching asset index…");
    ...
    if let Some(objects) = assets_json.get("objects").and_then(Value::as_object) {
        ...
        for (i, object) in objects.values().enumerate() {
            if i % 100 == 0 { emit_progress(&app, &instance, "assets", &format!("Downloading assets… {}/{}", i + 1, total)); }
    ...
    // mod loader: fabric only for now, forge/neoforge/quilt are on the roadmap (their installer model is a much bigger lift)
    let mut main_class = meta.get("mainClass").and_then(Value::as_str).ok_or("Minecraft main class missing")?.to_string();
    if loader == "fabric" {
        emit_progress(&app, &instance, "fabric", "Setting up Fabric…");
        let loader_version = match loader_version_pref {
            Some(v) => v,
            None => {
                let list = versions::list_fabric_loaders(mc_version.clone()).await?;
                list.first().map(|l| l.version.clone()).ok_or_else(|| format!("No Fabric loader builds found for Minecraft {}", mc_version))?
            }
        };
        let profile_url = format!("https://meta.fabricmc.net/v2/versions/loader/{}/{}/profile/json", mc_version, loader_version);
        let fabric_profile = fetch_json(&client, &profile_url).await?;
        main_class = fabric_profile.get("mainClass").and_then(Value::as_str)
            .ok_or("Fabric profile is missing a main class")?.to_string();
        if let Some(libs) = fabric_profile.get("libraries").and_then(Value::as_array) {
            for lib in libs {
                let (Some(name), Some(base_url)) = (lib.get("name").and_then(Value::as_str), lib.get("url").and_then(Value::as_str)) else { continue };
                let Some(rel) = maven_path(name) else { continue };
                let file = libraries_dir.join(&rel);
                let full_url = format!("{}/{}", base_url.trim_end_matches('/'), rel);
                download_file(&client, &full_url, &file, None).await?;
                classpath.push(file.to_string_lossy().to_string());
            }
        }
    } else if loader != "vanilla" {
        return Err(format!("{} isn't supported yet - Fabric and Vanilla work today, {} is on the roadmap", loader, loader));
    }
    ...
    let mut args = vec!["-Xmx4G".into(), format!("-Djava.library.path={}", natives_dir.to_string_lossy()), "-cp".into(), classpath.join(separator)];
    if let Some(acc) = &account { if acc.kind == "elyby" {
        emit_progress(&app, &instance, "identity", "Preparing ely.by skin/cape support…");
        let injector = ensure_authlib_injector(&client, &root).await?;
        args.push(format!("-javaagent:{}=ely.by", injector.to_string_lossy()));
    }}
    args.push(main_class);
    if let Some(jvm) = meta.get("arguments").and_then(|a| a.get("jvm")) { args.extend(collect_args(jvm, &game_dir, &assets_dir, &asset_id, &natives_dir, &instance, &mc_version, &username, &uuid, &access_token, &user_type)); }
    ...
    if !args.iter().any(|a| a == "--username") {
        args.extend(["--username".into(), username.clone(), "--uuid".into(), uuid, "--accessToken".into(), access_token, "--userType".into(), user_type, "--version".into(), mc_version.clone(), "--versionType".into(), "release".into(), "--gameDir".into(), game_dir.to_string_lossy().to_string(), "--assetsDir".into(), assets_dir.to_string_lossy().to_string(), "--assetIndex".into(), asset_id]);
    }

    emit_progress(&app, &instance, "starting", "Starting Java…");
    let mut child = Command::new("java").args(&args).current_dir(&game_dir).stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null()).spawn()
        .map_err(|e| format!("Could not start Java. Install Java 21 (or the right version for {}) or select a runtime in Settings. ({})", mc_version, e))?;
    let pid = child.id();
